  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (!authError) {
          setError('Check your email for a confirmation link. You can close this.');
          setLoading(false);
          return;
        }
      } else {
    ...
    <div className="flex min-h-screen items-center justify-center bg-bg px-4">
      <Card className="w-full max-w-md p-8">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-text-secondary">{isSignUp ? 'Create your account' : 'Sign in to your account'}</p>
        </div>
